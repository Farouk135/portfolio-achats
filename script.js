...
(function () {
  ...
  /* ----------------------------------------------------------------
     A PERSONNALISER : remplacez par l'URL EXACTE du profil LinkedIn
     d'Aya (copiez-la depuis le navigateur).
  ----------------------------------------------------------------- */
  var LINKEDIN_URL = "https://www.linkedin.com/in/aya-el-youssfi-alaoui";

  /* Coordonnees assemblees au chargement (limite la collecte par robots) */
  ...
  /* --------------------- Menu mobile (burger) ---------------------
     - transform anime dans les deux sens (jamais display:none)
     - fermeture au clic sur un lien, sur le voile, ou via Echap
     - verrouillage du defilement du corps quand le menu est ouvert
  ------------------------------------------------------------------ */
  function setupMobileMenu() {
    ...
    function openMenu() {
      ...
      scrim.hidden = false;
      void scrim.offsetWidth;              // reflow pour la transition d'opacite
    ...
    function closeMenu() {
      ...
      scrimTimer = window.setTimeout(function () { scrim.hidden = true; }, 350);
    }
    ...
    // Retour en mode bureau : on referme proprement
  ...
  /* ---------- Contact : assemblage sur des coordonnees ---------- */
  ...
  // Insere un texte en toute securite (textContent, jamais innerHTML)
